...
export class FileSyncService {
  ...
  async syncForStep(projectPath: string, stepId: string): Promise<void> {
    ...
    // Sync directory isolation hook (blocks access outside projectPath)
    this.syncDirectoryIsolation(projectPath)
  ...
  /**
   * Sync directory isolation: creates a PreToolUse hook that blocks
   * Read/Edit/Write/Glob/Grep/Bash from accessing paths outside the project directory.
   * Uses Claude Code's hook system for enforcement at the CLI level.
   */
  syncDirectoryIsolation(projectPath: string): void {
    const hookDir = join(projectPath, '.claude', 'hooks')
    const hookPath = join(hookDir, 'restrict-directory.sh')

    // The hook script receives JSON on stdin with tool_name and tool_input
    // Must return JSON with hookSpecificOutput.permissionDecision = "deny" to block
...
`
    ...
    // Create/update settings.json with the hook
